...
async function moderateImage(filePath, mimeType = 'image/jpeg') {
  ...
  if (result.flagged) {
    ...
    return {
      status: 'Flagged',
      reason: categories.length ? `Automatic safety review flagged: ${categories.join(', ')}` : 'Automatic safety review flagged this upload.',
    };
  }
